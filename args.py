...
arch_group.add_argument('--base', type=int,default=4) #'the layer to attach the first classifier'
arch_group.add_argument('--stepmode', type=str, choices=['even', 'lin_grow']) #'patten of span between two adjacent classifers |even|lin_grow|'
arch_group.add_argument('--step', type=int, default=1) #'span between two adjacent classifers'
arch_group.add_argument('--growthRate', type=int, default=6) #'number of output channels for each layer (the first scale)'
arch_group.add_argument('--grFactor', default='1-2-4', type=str) #growth rate factor of each sacle
arch_group.add_argument('--prune', default='max', choices=['min', 'max']) #specify how to prune the network, min | max'
arch_group.add_argument('--bnFactor', default='1-2-4') #bottleneck factor of each sacle, 4-4-4-4 | 1-2-4-4
